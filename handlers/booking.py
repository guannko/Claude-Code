...
def _admin_dismiss_kb(booking_id: int) -> InlineKeyboardMarkup:
    """Кнопка ОК для удаления уведомления из чата админа."""
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ ОК", callback_data=f"adm_notify:dismiss:{booking_id}"),
    ]])
...
async def cb_admin_booking_action(callback: CallbackQuery, bot: Bot) -> None:
    ...
    # ── Обновляем сообщение у админа: статус + кнопка ОК ─
    try:
        original = callback.message.text or ""
        await callback.message.edit_text(
            original + f"\n\n<b>{status_text}</b>",
            reply_markup=_admin_dismiss_kb(booking_id),
            parse_mode="HTML",
        )
    except Exception:
        pass
