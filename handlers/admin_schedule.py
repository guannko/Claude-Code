...
async def _masters_list_kb() -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="◀️ Закрыть", callback_data="admin:refresh")])
...
async def _build_master_text(master_id: str) -> str:
    ...
    for row in schedule:
        day = row["day_of_week"]
        if row["is_working"]:
            lines.append(f"✅ {_DAYS_SHORT[day]}  {row['start_time']}–{row['end_time']}")
        else:
    ...
    try:
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="◀️ Отмена", callback_data=f"adm_sch:master:{master_id}"),
            ]]),
        )
    except Exception:
        pass
...
async def msg_sch_hours(message: Message, bot: Bot, state: FSMContext) -> None:
    ...
    kb = _schedule_kb(master_id, schedule)
    try:
        await bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text=master_text, reply_markup=kb)
    except Exception:
        pass
